...
import { injectPatternBankRatingUI } from '../patternbank-api.js';
...
function loader(leetCode) {
  ...
  const intervalId = setInterval(async () => {
    try {
      ...
      // Trigger our overlay UI right after successful submission
      const difficultyMap = { 1: "Easy", 2: "Medium", 3: "Hard" };
      injectPatternBankRatingUI(
        problemName,
        leetCode.submissionData?.question?.questionFrontendId || problemName.split('-')[0],
        difficultyMap[leetCode.difficulty] || "Medium"
      );
