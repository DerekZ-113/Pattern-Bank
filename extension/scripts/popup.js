...
/* PATTERNBANK SYNC */
api.storage.local.get('patternbank_user_id', data => {
  if (data.patternbank_user_id) {
    $('#pb_user_id').val(data.patternbank_user_id);
  }
});

$('#save_pb_id').on('click', () => {
  const userId = $('#pb_user_id').val().trim();
  api.storage.local.set({ patternbank_user_id: userId }, () => {
    api.storage.sync.set({ patternbank_user_id: userId }, () => {
      $('#pb_status_msg').show().delay(2000).fadeOut();
    });
  });
});
