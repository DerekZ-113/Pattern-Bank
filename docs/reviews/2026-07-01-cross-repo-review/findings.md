...
**Status: FIXED 2026-07-01** — version 2 deployed from repo source (`verify_jwt=false` preserved), 401 smoke test passed, logs clean. Note: the deployed v1 matched no git commit (deployed from uncommitted state) — reinforces the migration/deploy-discipline recommendation in F-10.
...
**Status: FIXED 2026-07-01** — migration `20260702031459_widen_daily_review_goal_check_to_20` applied and verified (`CHECK 1..20`); first entry in the project's migration history (starts F-10 remediation). SQL recorded in `docs/supabase/widen-daily-review-goal.sql`.
